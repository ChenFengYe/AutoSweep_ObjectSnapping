...
namespace DataType
{
    public class KDTree
    {
        ...
        {
            float a = (float)points[inds[stIndex]][axis];
            float b = (float)points[inds[enIndex]][axis];
            int midIndex = (stIndex + enIndex) / 2;
            float m = (float)points[inds[midIndex]][axis];
        ...
        {
            float mySqDist = (float)(pivot - pt).magnitude;
            ...
            float planeDist = (float)(pt[axis] - pivot[axis]); //DistFromSplitPlane(pt, pivot, axis);
        ...
        {
            return (float)(pt[axis] - planePt[axis]);
        }
